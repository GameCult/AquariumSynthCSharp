...
public sealed class ZynReferenceParityTests
{
    ...
    public async Task ZynAddSubFxReferenceSourceIsPinnedForTestOnlyParity()
    {
        ...
        Assert.True(File.Exists(Path.Combine(zynRoot, "src", "Params", "PADnoteParameters.cpp")), "PAD parameter implementation is the parity source, not AquaSynth runtime code.");
        Assert.True(File.Exists(Path.Combine(zynRoot, "src", "Synth", "PADnote.cpp")), "PAD note implementation is the parity source, not AquaSynth runtime code.");
    ...
    public async Task ZynPadReferenceRendererWritesListeningArtifactsWhenBuilt()
    {
        ...
        var input = Path.Combine(root, "tests", "AquaSynth.Dsl.Tests", "Fixtures", "ZynAddSubFX", "ProjectAuthored", "pad-texture.xiz");
        ...
        var aquaWav = Path.Combine(artifactDir, "candidate-aquasynth.wav");
        ...
        var rebuild = ZynInstrumentReader.RebuildFirstPadAsAquaSynthScript(input, ParseBaseFrequency(tableReference.Stdout));
    ...
    public async Task ZynPadReferenceRendererSurveysProjectAuthoredPadFixturesWhenBuilt()
    {
        ...
        var fixtureDir = Path.Combine(root, "tests", "AquaSynth.Dsl.Tests", "Fixtures", "ZynAddSubFX", "ProjectAuthored");
        ...
        foreach (var input in Directory.GetFiles(fixtureDir, "pad-*.xiz").Order(StringComparer.OrdinalIgnoreCase))
        {
            ...
            var rebuild = ZynInstrumentReader.RebuildFirstPadAsAquaSynthScript(input, ParseBaseFrequency(table.Stdout));
    ...
    public async Task ZynPadReferenceRendererSurveysUpstreamGplPadFixturesWhenBuilt()
    {
        ...
        foreach (var relative in selected)
        {
            ...
            var tableRaw = Path.Combine(runDir, "reference-zyn-table0.f32");
            var tableWav = Path.Combine(runDir, "reference-zyn-table0.wav");
            var aquaWav = Path.Combine(runDir, "candidate-aquasynth.wav");
            ...
            var rebuild = ZynInstrumentReader.RebuildEnabledPadsAsAquaSynthScript(input, tableRoots);
            await File.WriteAllTextAsync(Path.Combine(runDir, "candidate.aqua"), rebuild.Script);
