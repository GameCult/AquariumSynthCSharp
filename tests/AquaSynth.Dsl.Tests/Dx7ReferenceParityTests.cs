...
public sealed class Dx7ReferenceParityTests
{
    ...
    public void ProjectAuthoredDx7EnvelopeTraceWritesComparisonArtifact()
    {
        ...
        var aquasynth = TraceRateLevelEnvelope(approximation.Envelope, approximation.GateSeconds, durationSeconds: 1.1f);
        var aquasynthCurved = TraceRateLevelEnvelope(
        ...
        writer.WriteLine("time_seconds,dx7_raw_gain,dx7_applied_gain,aquasynth_rl_gain,aquasynth_curved_gain,dx7_stage");
        for (var i = 0; i < Math.Min(dx7.Count, aquasynth.Length); i += 64)
        {
        ...
        var curvedShape = NormalizedDistance(
            dx7Applied.Select(point => point.Gain).ToArray(),
            aquasynthCurved);
        Assert.True(linearShape > 0.55f, $"expected current AquaSynth envelope to visibly differ; shape distance {linearShape}, artifact: {path}");
        Assert.True(curvedShape < linearShape * 0.6f, $"expected curved AquaSynth envelope to improve shape; linear {linearShape}, curved {curvedShape}, artifact: {path}");
    }
        ...
        AudioComparison comparison)
    {
        ...
        WriteWav(Path.Combine(directory, "candidate-aquasynth.wav"), candidate, sampleRate, scale);
        ...
        File.WriteAllText(
            ...
            string.Join(
                ...
                $"candidate: {Path.Combine(directory, "candidate-aquasynth.wav")}"));
    }
...
internal static class DexedPyRenderer
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var tempDir = Path.Combine(Path.GetTempPath(), $"aquasynth-dx7-render-{Guid.NewGuid():N}");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var tempDir = Path.Combine(Path.GetTempPath(), $"aquasynth-dx7-patch-render-{Guid.NewGuid():N}");
        Directory.CreateDirectory(tempDir);
        try
        {
    ...
    {
        var configuredPython = Environment.GetEnvironmentVariable("AQUASYNTH_DX7_PYTHON");
